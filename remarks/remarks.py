...
def run_remarks(
    input_dir, output_dir, file_name=None, file_uuid=None, file_path=None, md_hl_output_dir=None, **kwargs
):
    ...
    logging.info(
        f'\nFound {num_docs} total documents in "{input_dir}"',
    )
    ...
    for metadata_path in pathlib.Path(f"{input_dir}/").glob("*.metadata"):
        ...
        if doc_type in supported_types:
            logging.debug(f'\nFound File: "{doc_name}.{doc_type}" ({metadata_path.stem})')

            in_device_dir = get_ui_path(metadata_path)
            out_path = pathlib.Path(f"{output_dir}/{in_device_dir}/{doc_name}/")

            if md_hl_output_dir is not None:
                out_path_highlights = pathlib.Path(f"{md_hl_output_dir}/{doc_name}/")
            else:
                out_path_highlights = out_path
            ...
            process_document(metadata_path, out_path, out_path_highlights, doc_type, **kwargs)
        else:
            logging.info(
...
def process_document(
    ...
    for page_uuid in pages_to_process:
        ...
        if combined_md and (has_ann_hl or has_smart_hl):
            ...
            combined_md_strs += [(label if label else page_idx + 1 + md_page_offset, hl_text + "\n")] # page_idx is zero-indexed
    ...
    if combined_md and len(combined_md_strs) > 0:
        ...
        if md_header_format == "atx":
            ...
            if md_obsidian_format:
                obsidian_template_str = "".join([
                    "" 
                        if "title" not in pdf_src.metadata or pdf_src.metadata["title"] == "" 
                        else f"# {pdf_src.metadata['title']}\n",
                    "" 
                        if "author" not in pdf_src.metadata or pdf_src.metadata["author"] == "" 
                        else f"{pdf_src.metadata['author']}\n",
                    "\n#remarks-tool #highlights #do-not-edit" 
                    "\n\n---\n",
                    "\n*This file is auto-generated by the Remarks tool. Any manual edits may be overridden, and some text may be rendered improperly. Please consult the original document for exact quotes and page numbers.*"
                    "\n\n---\n",
                ])
                combined_md_str = obsidian_template_str + combined_md_str
            else:
        ...
        with open(f"{os.path.splitext(f'{out_path_highlights.parent}/{out_path_highlights.name}')[0]}_highlights_remarks.md", "w") as f:
